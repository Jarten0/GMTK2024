	...
	if !Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and dragging:
		...
		if colliding_area.size() != 0:
			match type:
				...
				2: colliding_area[0].scale_jumbo()
				3: colliding_area[0].scale_mini()
				4: colliding_area[0].scale_up_x()
				5: colliding_area[0].scale_down_x()
				6: colliding_area[0].scale_up_y()
				7: colliding_area[0].scale_down_y()
